...
namespace ORACLEDAL
{
    public class ORACLEDataAccess : IDataAccess
    {
        ...
        /// <summary>
        /// 将传入的参数转换为OracleParameter类型，并将命令中的占位符用实际值替换
        /// </summary>
        /// <param name="parameters"></param>
        /// <returns></returns>
        protected OracleParameter[] ConvertParam(object[] parameters, ref string commandText, CommandType commandType)
        {
            ...
            List<OracleParameter> sqlParamList = new List<OracleParameter>();
            foreach (object p in parameters)
            {
                OracleParameter sqlP = null;
                if (p is IDataParameter)
                {
                    IDataParameter dbP = (IDataParameter)p;
                    sqlP = new OracleParameter(dbP.ParameterName, dbP.Value);
        ...
        /// <returns></returns>
        protected string GetNoParamCommandText(string commandID, string commandText, OracleParameter[] param)
        {
        ...
        public int ExecNonQueryTran(string commandID, params object[] parameters)
        {
            ...
            using (OracleConnection connection = new OracleConnection(ConnectionString))
            {
                ...
                try
                {
                    ...
                    string cmdText = GetCommandText(commandID);
                    CommandType commandType = GetCommandType(commandID);
                    OracleParameter[] param = (parameters == null ? null : ConvertParam(parameters, ref cmdText, commandType));

                    Util.Log.Debug(GetNoParamCommandText(commandID, cmdText, param));

                    retval = ORACLEHelper.ExecuteNonQuery(transaction, GetCommandType(commandID), cmdText, param);
        ...
        public DataSet FillDataSet(string commandID,  params object[] parameters)
        {
            ...
            return ORACLEHelper.ExecuteDataset(ConnectionString, commandType, cmdText, param);
        }
        ...
        public IDataReader ExecDataReader(string commandID,  params object[] parameters)
        {
            ...
            return ORACLEHelper.ExecuteReader(ConnectionString, commandType, cmdText, param);
        }
